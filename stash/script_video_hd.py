...
def process_video(video_path, frame_output_folder, conf_threshold=0.4):
    ...
    for results in track_results:
        ...
        if total_frames > 0:
            completion_ratio = min(frame_count / total_frames, 1.0)
            completed_percent = int(completion_ratio * 100)

            while next_progress_percent <= completed_percent and next_progress_percent <= 100:
                elapsed_seconds = time.time() - processing_start_time
                ...
                if completion_ratio > 0:
                    ...
                    expected_done_at = datetime.fromtimestamp(time.time() + remaining_seconds).strftime("%Y-%m-%d %H:%M:%S")
                else:
                    expected_done_at = "N/A"

                print(
                    f"[Progress] {next_progress_percent}% completed | "
                    f"elapsed: {format_duration(elapsed_seconds)} | "
                    f"expected done: {expected_done_at}"
                )
